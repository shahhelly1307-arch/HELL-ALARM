#!/usr/bin/env node
/**
 * One-command setup for pixel pomodoro.
 *
 *   npm run setup
...
async function manualExtractElectron() {
  ...
  const writePathTxt = () => {
    try {
      fs.writeFileSync(path.join(ELECTRON_DIR, 'path.txt'), platformExePath())
    } catch {
      // best effort
    }
  }

  // Attempt 1 — extract-zip (pure JS, ships as a dependency of electron itself).
  try {
...
async function repairElectron() {
  ...
  if (electronBinaryOk()) {
    ...
    if (zip) {
      ...
      warn('Full step-by-step is in the Troubleshooting section of README.md.')
    } else {
      warn('The binary never downloaded — check your network/antivirus, or switch')
      warn('to Node 24.x (LTS) and re-run `npm run setup`. See README.md → Troubleshooting.')
    }
    process.exit(1)
  }
}
...
// --- go ---------------------------------------------------------------------
async function main() {
  console.log(`${c.bold}pixel pomodoro — setup${c.reset}`)
  checkNode()
  installDeps()
